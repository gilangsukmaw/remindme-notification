...
import { GET_NOTE_BEGIN, GET_NOTE_SUCCESS, GET_NOTE_FAIL, GET_NOTEDETAIL_BEGIN, GET_NOTEDETAIL_SUCCESS, GET_NOTEDETAIL_FAIL } from "../const/type";
...
function* getNoteDetail(actions) {
  const { id } = actions;
  try {
    const res = yield axios.get(`https://remindme.gabatch13.my.id/api/v1/notes/${id}`, { headers: { Authorization: `Bearer ${Token}` } });
