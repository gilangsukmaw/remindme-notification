...
function AddProgressModal(props) {
  ...
  function AddProgressModal(props) {
    ...
    const submitAddProgress = async (e) => {
      try {
        const res = await axios.post(
          `https://remindme.gabatch13.my.id/api/v1/goals/${id}`,
          state,
    ...
    return (
      <>
        <Modal
          className="AddProgressModal shadow"
          {...props}
          size="lg"
        ...
        >
          ...
          <Modal.Title
            className="AddProgressTitle "
            id="contained-modal-title-vcenter"
          >
            <img
              className="MainTitle"
              alt="remindme"
              style={{ width: "34px", height: "34px", marginRight: "1rem" }}
              src={Logo}
            ></img>
            <div className="MainTitle">Add Progress</div>
          </Modal.Title>
          <div
            className="SetProgress"
            style={{
              marginBottom: "-6rem",
              fontSize: "22px",
              fontWeight: "700",
          ...
          <Modal.Body className="text-dark">
            <Form onSubmit={submitAddProgress} className="ProgressValue">
              <Form.Group
                className="ValuePlaceholderLeft PlaceHolder"
                controlId="enter value"
              >
                <Form.Control
                  style={{
                    float: "right",
                  ...
                  onChange={(e) =>
                    setState({ ...state, progress: parseInt(e.target.value) })
                  }
                />
              </Form.Group>
              <Form.Group
                className="ValuePlaceholderRight PlaceHolder"
                controlId="enter value"
              >
                <Form.Control
                  disabled
  ...
  return (
    <div className="AddProgressButton" style={{ marginBottom: "3.5rem" }}>
      ...
      <Button
        ...
        onClick={() => setProgressShow(true)}
      >
        <img
          className="AddProgressImg"
          alt="remindme"
          style={{ width: "34px", height: "34px", marginRight: "1rem" }}
          src={Logo}
        ></img>
        <div style={{ color: "Black" }} className="AddProgressText">
          Add Progress{" "}
        </div>
      </Button>
      <AddProgressModal
