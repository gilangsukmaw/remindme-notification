...
function AllGoals({ ...props }) {
  ...
  function logo() {
    if (details?.target_type === "Mililiter") {
      return droplet;
    }
    if (details?.target_type === "Kilometer") {
      return length;
    }
    if (details?.target_type === "Meter") {
      return length;
    }
    if (details?.target_type === "Hours") {
      return clock;
    }
    if (details?.target_type === "Minutes") {
      return clock;
    }
    if (details?.target_type === "Liter") {
      return droplet;
    }
    if (details?.target_type === "Times") {
      return clock;
    }
    if (details?.target_type === "Plate") {
      return plate;
    }
    if (details?.target_type === "Drink") {
      return droplet;
    }
    if (details?.target_type === "Other") {
      return ProgressLogo;
    }
  }
    ...
    const submitGoals = async (e) => {
      try {
        const res = await axios.put(
          `https://remindme.gabatch13.my.id/api/v1/goals/${details?.id}`,
          state,
          { headers: { Authorization: `Bearer ${Token}` } }
        );
        setEditModal(false);
                ...
                <OverlayTrigger
                  ...
                  overlay={
                    <Tooltip id="tooltip-disabled">
                      You already choose "{details?.goal_type}"
                    </Tooltip>
                  }
                >
                  <span style={{ width: "45%" }}>
  ...
  return (
    <>
      <div
        className="bungkus"
        style={{ marginLeft: "400px", paddingTop: "2rem" }}
      >
        {/* ==================mapping All Goals=================== */}
        ...
        <div
          ...
          className="GoalsContainer d-flex overflow-auto "
        >
          {goals?.data
        ...
        <div>
          <img
            style={{ width: "100%", height: "15px", padding: "0 2rem 0 2rem" }}
            src={Garis}
            alt=""
          />
        </div>
        ...
        {details?.length === 0 ? null : (
          <div
            className="GoalDetails d-flex flex-row justify-content-between"
          ...
          <Container className="DetailBox ">
            <Col lg={6} md={12}>
              <div className="mappingDetailGoals">
                {details?.length === 0 ? null : details?.current_percent ===
                  undefined ? (
                  ...
                  <DetailCircular
                    ...
                    currentProgress={details?.current}
                    currentPercentage={details?.current_percent}
                    type={details?.target_type}
                    id={details?.id}
                  />
                )}
              </div>
                      ...
                      <div className="mappingGoals">
                        <p>
                          <img
                            ...
                            alt=""
                          />
                          {item?.progress} {details?.target_type}
                        </p>
                      </div>
                    </div>
                  </Container>
